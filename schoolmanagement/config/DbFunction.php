...
class DbFunction{
	...
	if(false==$stmt){
		...
		if(!$rs)
		{
			...
			echo "<script>alert('valid Detail')</script>";
	...
	function create_course($cshort,$cfull,$cdate){
			...
			if(false===$stmt){
				...
				$stmt->execute();
				echo "<script>alert('Elective Subject Added Successfully')</script>";
...
function register($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
   ...
   $query = "INSERT INTO `registration` (`course`, `subject`, `fname`, `mname`, `lname`, `gender`, `gname`, `ocp`,
			...
			$stmt->bind_param('ssssssssssssssssssssssssssssssss',
					...
					$sub1,$sub2,$marks1,$marks2,$fmarks1,$fmarks2,$reg);
			$stmt->execute();
